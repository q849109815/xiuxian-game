...
const EX = {
  ...
  skins: [
    ...
    { id: 'sk_c04b', char: 'C04', n: '暗夜', icon: '🥷', img: 'assets/skin/sk_c04b.jpg', price: 680, bonus: { crit: 0.10 }, desc: '暴击 +10%' },
  ...
  levels: [
    ...
    { id: '10-10', ch: 10, n: 'BOSS·尸王', waves: 20, pool: ['putong', 'jipao', 'zhongjia', 'du', 'tuye', 'zhadan', 'dun', 'fenlie', 'feixing', 'jinying'], per: [26, 42], mul: 131.75,
      cond: 'boss', boss: 'shiwang', rw: { gold: 9573500, M01: 28 }, unlock: '10-9', scene: 'factory' },
  ],
  ...
  HOT_UPDATE: {
    enabled: true,
    version: '20260926n',      /* 主版本号，改这个会强制全量刷新缓存 */
    /* 资源清单：path / ver / size(KB) */
    manifest: [
      { p: 'css/style.css', ver: '20260926n', size: 0 },
      { p: 'js/net.js', ver: '20260926n', size: 0 },
      { p: 'js/config.js', ver: '20260926n', size: 0 },
      { p: 'js/engine.js', ver: '20260926n', size: 0 },
      { p: 'js/battle.js', ver: '20260926n', size: 0 },
      { p: 'js/ui.js', ver: '20260926n', size: 0 },
      { p: 'js/main.js', ver: '20260926n', size: 0 },
      { p: 'js/audio.js', ver: '20260926n', size: 0 },
    ],
  },
  ...
  achShop: [
    ...
    { id: 'AS05', n: '急救包x1', t: '消耗', cost: 80, limit: 3, per: 'day', need: 0, give: { I01: 1 } },
    { id: 'AS06', n: '护盾发生器x1', t: '消耗', cost: 100, limit: 2, per: 'day', need: 0, give: { I02: 1 } },
  ...
  shopGoods: {
    ...
    '礼包': [
      ...
      { id: 'GP05', n: '月度超值', icon: '👑', img: 'assets/icon/gp05.jpg', price: 680, cur: 'diamond', rmb: '68元',
  ...
  /* 军团商店：消耗军团贡献
   * 此前捐献花 5000 金币换 100 贡献，但「军团商店」按钮只弹 toast
   * → 贡献赚了没有任何出口，等于白捐。 */
  legionShop: [
    /* 军团贡献 ↔ 金币 汇率（严重经济漏洞修复）
     * 捐献：5000 金 → 100 贡献  ⇒  50 金/贡献
     * 原 LS01：60 贡献 → 5000 金  ⇒  83.33 金/贡献
     * → 循环倍率 83.33 / 50 = 1.667，且 lim=0（无限兑）
     * → 金币可以反复「捐献 → 兑金币袋」凭空增殖，实测 10 万 → 20 万。
     * 现在把金币袋汇率压到明确低于捐献汇率（2500/60 = 41.7 金/贡献，循环 0.83），
     * 贡献仍可折回金币（捐多了不浪费），但每循环一次净亏约 17%，套利链断开。 */
    { id: 'LS01', n: '金币袋', icon: '💰', cost: 60, item: 'gold', n2: 2500, lim: 0 },
    { id: 'LS02', n: '合金', icon: '🔩', cost: 40, item: 'M02', n2: 10, lim: 0 },
    { id: 'LS03', n: '稀有金属', icon: '💠', cost: 120, item: 'M03', n2: 5, lim: 3 },
    { id: 'LS04', n: '枪械碎片', icon: '🧩', cost: 150, item: 'P01', n2: 5, lim: 2 },
    { id: 'LS05', n: '普通芯片', icon: '🔲', cost: 200, item: 'C01', n2: 1, lim: 2 },
    { id: 'LS06', n: '红宝石', icon: '🔴', cost: 400, item: 'gem_G_R', n2: 1, lim: 1 },
  ],
  /* 军团副本：消耗体力，产出贡献与奖励
   * 此前「军团活动」按钮写死「开发中」，点了没反应。 */
  legionActs: [
    { id: 'LA01', n: '尸潮防线', icon: '🧟', cost: 15, need: 5000,
      contrib: 80, gold: 2000, mats: [{ k: 'M01', n: 8 }] },
    { id: 'LA02', n: '物资争夺', icon: '📦', cost: 20, need: 15000,
      contrib: 150, gold: 4000, mats: [{ k: 'M02', n: 6 }] },
    { id: 'LA03', n: '母巢攻坚', icon: '🕸️', cost: 25, need: 40000,
      contrib: 260, gold: 8000, mats: [{ k: 'M03', n: 4 }, { k: 'M05', n: 3 }] },
  ],
  /* 【军团】社交：加入/自建/捐献 */
  legions: [
  ...
  /* 【远征堡垒】消耗体力挑战 */
  expeds: [
    /* BUG修复：产出原先是 `M0${random(1..5)}` 全随机，
     * ① 与本条 desc 完全对不上（写"产稀有晶体"却可能给金属）
     * ② 面板总描述写"产出稀有材料与宝石"，但代码从不发宝石 —— 实测 20 次 0 宝石，
     *    玩家花 10~20 体力打远征永远拿不到宝石，宝石只剩商城一条路。
     * 现在按各副本描述精确配置 mats，并给 E02/E03 配宝石产出。 */
    { id: 'E01', n: '尸潮围剿', icon: '🧟', cost: 10, need: 3000, gold: 800, mat: 5,
      desc: '成波次尸潮，产出金属与合金',
      mats: [{ k: 'M01', n: 5 }, { k: 'M02', n: 2 }] },
    { id: 'E02', n: '精英猎杀', icon: '☠️', cost: 15, need: 8000, gold: 1500, mat: 8,
      desc: '精英僵尸，产出稀有金属与宝石',
      mats: [{ k: 'M03', n: 3 }],
      gem: { rate: 0.35, n: 1 } },
    { id: 'E03', n: '母巢突袭', icon: '🕸️', cost: 20, need: 20000, gold: 3000, mat: 12,
      desc: '深入母巢，产出火药、电子元件与宝石',
      mats: [{ k: 'M04', n: 5 }, { k: 'M05', n: 3 }],
      gem: { rate: 0.55, n: 1 } },
  ],
  /* 巡逻：每小时产出随章节提升 */
   ...
   * =================================================== */
  guides: [
    /* BUG：摇杆已按需求移除（左下摇杆元素/CSS/JS 三处全部清理），
     *   但引导 1 仍写着「使用左下摇杆移动角色」——
     *   新玩家进游戏第一眼看到的引导，教的却是一个屏幕上根本不存在的控件。
     *   同时引导 2 的触发时机写的是「移动后」，而「移动」这个操作已不存在。
     *   两条文案一并改为贴合当前（自动迎战 + 自动瞄准）的实际操作。 */
    { id: 1, n: '战斗引导', trig: '进入第一关', txt: '角色自动迎战，怪物从右侧不断来袭', way: '半透明手指指示', must: true },
    { id: 2, n: '射击引导', trig: '进入战斗后', txt: '自动瞄准射击，无需手动操作', way: '箭头指向+文字', must: true },
  ...
  /* 局内技能伤害总系数
   * 背景：11 个主动/召唤技能此前「只显示等级、效果为零」，修复后强度未校准，
   *   实测只需自动释放 3~5 次即可碾压：100 关全量扫描通关率 100%，
   *   最终关 10-10 也从 3/10 跳到 9/10 —— 完全没有卡点。
   * 技能伤害 = atk × rwMul × 各技能系数 × 本值，统一收在这里便于调平衡。 */
  SKILL_DMG_K: 0.45,

  /* 战斗内 3D 卡片：僵尸 / 主角 / 武器均以立体卡牌呈现
   * （卡牌厚度侧面 + 品质描边 + 摆动 + 扫光 + 地面投影）
   * 设为 false 则退回原来的扁平造型 + 贴地阴影（低端机可关以省性能） */
  CARD3D: false,            /* 战斗内的"竖立卡牌"模式：卡牌本身就是纸片感的来源，默认关闭 */
  ...
  /* 章节过场文案（配合 CG 展示） */
  chapterDesc: {
    1: '尸潮初现，城市沦陷。守住最后一道防线。',
    4: '深入工厂区，机械与尸群交织的死亡地带。',
    7: '废墟之上重建据点，人类最后的希望。',
    10: '最终决战，直面尸王的老巢。',
  },
